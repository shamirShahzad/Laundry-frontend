...
const AddItems = () => {
  ...
  const handleSubmit = async (formData: AddItemFormData) => {
    setLoading(true);
    try {
      const pricesObject = formData.prices.reduce((acc, curr) => {
        acc[curr.serviceId] = curr.price;
        return acc;
      ...
      data.append("price", JSON.stringify(pricesObject)); // stringified prices object
      if (formData.image) {
                    ...
                    <>
                      ...
                      <div className="flex mt-4 pb-6 ml-9">
                        <Button
                          ...
                          onClick={() => push({ serviceId: "", price: 0 })}
                        >
